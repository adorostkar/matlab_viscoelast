...
for k=1:nip                         
    FUND_all(1:4,k)   = shape_fun_quad(Gauss_point,k);    % FUN(1,np)
    DERD_all(1:2,1:4,k) = shape_der_quad(Gauss_point,k);    % DER(2,np),bilinear b.f.
    FUNP_all(1:4,k)   = shape_fun_quadTH(Gauss_point,k);
end
...
while (norm_cur>1e-6)&(time_length<=Tmax)
    k = k + 1;
   ...
   if(verbose ~= 0)
       figure(2),hold on
       plot(Node(1,Surface_Nodes), Node(2,Surface_Nodes),'.')
       hold off
%       figure(3),clf,Bvisual_mesh(Node,Edge,Face,1,1,1,0,16),wait
   end
...
%    if  (delta_t_prev~=delta_t_cur)
        S_cur       = S - theta*delta_t_cur*So;
        [S_cur,K,A] = Dirichlet_Esdo_matrix(S_cur,K,A,...
